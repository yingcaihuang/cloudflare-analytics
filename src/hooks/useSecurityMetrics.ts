 ...
 * Custom hook for fetching and managing security metrics data
...
import { SecurityMetrics, MetricsQueryParams } from '../types';

interface UseSecurityMetricsResult {
...
const CACHE_KEY_PREFIX = 'security_metrics_';
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

/**
 * Custom hook for fetching security metrics
...
export function useSecurityMetrics(
  ...
  const fetchData = useCallback(async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      if (!forceRefresh) {
        const cachedData = await CacheManager.getData<SecurityMetrics>(cacheKey);
        if (cachedData) {
      ...
      // Fetch from API
      const result = await GraphQLClient.querySecurityMetrics(params);

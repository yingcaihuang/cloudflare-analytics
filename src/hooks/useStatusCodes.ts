/**
 * useStatusCodes Hook
 * Custom hook for fetching and managing HTTP status code distribution data
 * Implements data loading with GraphQL + caching strategy
 */

import { useState, useEffect, useCallback } from 'react';
import GraphQLClient from '../services/GraphQLClient';
import CacheManager from '../services/CacheManager';
import { StatusCodeData, MetricsQueryParams } from '../types';

interface UseStatusCodesResult {
  data: StatusCodeData | null;
  loading: boolean;
  error: string | null;
...
const CACHE_KEY_PREFIX = 'status_codes_';
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

/**
 * Custom hook for fetching status code distribution
...
export function useStatusCodes(
  ...
  const fetchData = useCallback(async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      if (!forceRefresh) {
        const cachedData = await CacheManager.getData<StatusCodeData>(cacheKey);
        if (cachedData) {
      ...
      // Fetch from API
      const result = await GraphQLClient.queryStatusCodes(params);
      ...
      if (cachedData) {
        ...
        setError(err instanceof Error ? err.message : 'Failed to fetch status codes');
      }
    } finally {
      setLoading(false);
    }

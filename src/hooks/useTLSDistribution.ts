/**
 * useTLSDistribution Hook
 * Custom hook for fetching and managing TLS version distribution data
...
import { TLSData, MetricsQueryParams } from '../types';

interface UseTLSDistributionResult {
  data: TLSData | null;
  loading: boolean;
  error: string | null;
...
const CACHE_KEY_PREFIX = 'tls_distribution_';
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

/**
 * Custom hook for fetching TLS version distribution data
 ...
 */
export function useTLSDistribution(
  params: MetricsQueryParams,
  autoFetch: boolean = true
): UseTLSDistributionResult {
  ...
  const fetchData = useCallback(async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      if (!forceRefresh) {
        const cachedData = await CacheManager.getData<TLSData>(cacheKey);
        if (cachedData) {
      ...
      // Fetch from API
      const result = await GraphQLClient.queryTLSDistribution(params);
      ...
      if (cachedData) {
        ...
        setError(err instanceof Error ? err.message : 'Failed to fetch TLS distribution');
      }
    } finally {
      setLoading(false);
    }
